...
class Transformation:
	...
	def get_qr_code(self):
		if self.code is not '':
			self.listener.waitForTransform("/map", "/qr_frame", rospy.Time(), rospy.Duration(4.0))
			try:
			...
			if qr_name not in self.trs:
				self.trs[qr_name] = [trans1, float(self.sec[0][1]), float(self.sec[1][1]), float(self.sec[2][1]), float(self.sec[3][1]),
